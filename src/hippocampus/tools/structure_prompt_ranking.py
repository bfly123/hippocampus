...
def rank_source_files(
    ...
    module_scores = {module.get("id", ""): float(module.get("core_score", 0.0)) for module in modules}
    ranked: list[tuple[str, dict, float]] = []

    for file_path, file_data in files.items():
        if file_roles.get(file_path) != ROLE_SOURCE:
            continue
        ...
        if any(
            ...
            for marker in ("/gateway/", "/router", "/server", "/nav/", "/memory/", "/mcp/")
        ):
            score += 3.0
        ...
        if "test" in name or any(part.lower() in _TEST_DIRS for part in parts):
            score -= 12.0

        if score > 0:
            ranked.append((file_path, file_data, score))

    ranked.sort(key=lambda item: (item[2], len(item[1].get("signatures", [])), item[0]), reverse=True)
    return ranked
...
def is_noise_file_path(file_path: str) -> bool:
    ...
    if name in _NOISE_FILE_NAMES:
        return True
    if any(name.endswith(suffix) for suffix in _NOISE_FILE_SUFFIXES):
        return True
    return False
